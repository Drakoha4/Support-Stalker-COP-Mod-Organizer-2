...
from .stalker import XRSave


class StalkerModDataChecker(mobase.ModDataChecker):
    ...
    def findLostData(self, tree: mobase.IFileTree) -> list[mobase.FileTreeEntry]:
        ...
        for e in tree:
            if e.isFile():
                if e.suffix().lower().startswith("db", "xdb"):
                    lost_db.append(e)
    ...
    def fix(self, filetree: mobase.IFileTree) -> mobase.IFileTree:
        ...
        if lost_db:
            rfolder = filetree.addDirectory("db", "xdb").addDirectory("mods")
            for r in lost_db:
...
class StalkerModDataContent(mobase.ModDataContent):
    content: list[int] = []
...
class StalkerSaveGame(BasicGameSaveGame):
    _filepath: Path
...
class StalkerSaveGameInfoWidget(mobase.ISaveGameInfoWidget):
    ...
    def setSave(self, save: mobase.ISaveGame):
        self.resize(240, 32)
        if not isinstance(save, StalkerSaveGame):
            return
...
class StalkerSaveGameInfo(BasicGameSaveGameInfo):
    def getSaveGameWidget(self, parent: QWidget | None = None):
        return StalkerSaveGameInfoWidget(parent)
...
class StalkerGame(BasicGame, mobase.IPluginFileMapper):
    ...
    GameName = "STALKER"
    GameShortName = "stalker"
    GameNexusName = "stalker"
    GameNexusId = 3743
    GameBinary = "Stalker-COP.exe"
    GameDataPath = ""
    ...
    def init(self, organizer: mobase.IOrganizer):
        ...
        self._featureMap[mobase.ModDataChecker] = StalkerModDataChecker()
        self._featureMap[mobase.ModDataContent] = StalkerModDataContent()
        self._featureMap[mobase.SaveGameInfo] = StalkerSaveGameInfo()
    ...
    def executables(self) -> list[mobase.ExecutableInfo]:
        info = [
            ["COP Launcher", "Stalker-COP.exe"],
            ["AdvancedXRay", "bin/AdvancedXRay.exe"],
            ["COP xrEngine", "bin/xrEngine.exe"],
        ]
        gamedir = self.gameDirectory()
        return [
